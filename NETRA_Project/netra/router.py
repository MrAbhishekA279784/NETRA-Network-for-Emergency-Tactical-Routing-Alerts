...
from netra.ml.predict import predict_cost_grid
...
def generate_routes(layout_payload: dict, force_mode: Optional[str] = None) -> dict:
    """
    Main NETRA routing function.

    Mode support:
      "classic": Uses rule-based cost grid
      "ml": Uses AI-predicted risk per person

    Pipeline
    --------
    1. Parse JSON payload → typed BuildingLayout
    2. Build weighted base cost grid
    3. Filter to open exits
    4. Sort persons by priority descending (critical first)
    5. For each person, optionally predict ML grid
    6. A* to every open exit → pick best (cost + congestion)
    7. Update exit load after each assignment
    8. Detect & flag trapped persons
    9. Return structured JSON result
    """
    t0 = time.perf_counter()

    mode = force_mode if force_mode else layout_payload.get("mode", "classic")

    # 1. Parse
    layout = parse_layout(layout_payload)

    # 2. Base cost grid
    base_cost_grid = build_cost_grid(layout)
    ...
    for person in sorted_persons:
        if mode == "ml":
            active_grid = predict_cost_grid(layout, person, base_cost_grid)
        else:
            active_grid = base_cost_grid
            
        route = _select_best_exit(
            person, open_exits, active_grid,
